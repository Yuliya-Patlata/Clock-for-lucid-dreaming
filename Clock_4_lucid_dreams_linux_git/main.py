...
class FirstRingError(Exception):
    def __init__(self, first_ring, wake_up):
        Exception.__init__(self)
        self.first_ring = first_ring
...
class LucidIntervalError(Exception):
    def __init__(self, first_ring, lucid_interval,  wake_up):
        Exception.__init__(self)
        self.first_ring = first_ring
        self.lucid_interval = lucid_interval
        self.wake_up = wake_up
        print('Input shorter lucid_interval , othewise quite ring will never sound')

class LucidDurationError(Exception):
    def __init__(self, first_ring, lucid_duration,  wake_up):
        Exception.__init__(self)
        self.first_ring = first_ring
        self.lucid_duration = lucid_duration
        self.wake_up = wake_up
...
if __name__=="__main__":
    try:
        wake_up_str = input('Input wake up time in format "00:00:00", для  тестирования указвать wake_up \n на 4-7 минут больше текущего \n wake_up_str => ')
        wake_up = convert_ring(wake_up_str)
        print(type(wake_up), wake_up)

        lucid_duration = float(input('Input total aproximate planned duration \n of lucid_dreaming in hours \n ( для  тестирования указывать lucid_duration 2-5.1 минут): lucid_duration => '))

        first_ring = wake_up - timedelta( minutes = lucid_duration) #( для теста указаны  минуты ,  а вообще  будут  часы hours = lucid_duration)
        print('The time of first quiet ring will be :', first_ring)

        lucid_interval = int(input('Input interval of quiet rings in \n minutes, which will ring throught planned \n duration of lucid_dreaming lucid_interval \n ( для  тестирования указывать lucid_interval 150 - 170 секунд)  =>'))
        ...
        if first_ring >= wake_up or first_ring <= cur_time:
            raise FirstRingError(first_ring, wake_up)

        if first_ring + timedelta(seconds=lucid_interval) >= wake_up:
            raise LucidIntervalError(first_ring, lucid_interval,  wake_up)

        if wake_up - timedelta(minutes=lucid_duration) <= datetime.now() :
            raise LucidDurationError(first_ring, lucid_duration,  wake_up)
        ...
        while cur_time <= wake_up:
            if next_ring + timedelta( seconds = quiet_sound_duration) < wake_up:
                ...
                if cur_time == next_ring:
                    ...
                    next_ring = next_ring + timedelta(
                        seconds=lucid_interval)  # для  тестирования тут секунды,а вообще будут минуты
                    print('next_quiet_ring will be :', next_ring)
                    # system('vargan_low.mp3') # for windows играет в  бесконечном цикле без остановки
